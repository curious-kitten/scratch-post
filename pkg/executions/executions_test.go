...
func TestExecution_Validate(t *testing.T) {
	...
	g.Expect(decoder.IsValidationError(err)).To(BeTrue(), "empty execution error is not a validation error")
	...
	g.Expect(decoder.IsValidationError(err)).To(BeTrue(), "execution without a scenario ID is not a validation error")
	...
	err = s.Validate()
	g.Expect(err).Should(HaveOccurred(), "No error with execution that does not have a project ID")
	g.Expect(decoder.IsValidationError(err)).To(BeTrue(), "execution without a test plan ID is not a validation error")
